...
def get_attn_map(attn_cache, attn_inds=None, block_id=f'transformer_blocks.2', 
                 token_type = 'text',
                 height=960, width=960, token_C=2,
                 embed_size = 0, t_0=0, t_1=25):
    ...
    attention_map = attention_probs[:,:,noise_inds[0][0]:noise_inds[0][1],:]
    ...
    if token_type == 'text':
        ...
        attn_inds = list(range(clean_latent_inds[token_C][0], clean_latent_inds[token_C][1]))
    else:
        attn_inds = attn_inds
...
def get_img_attn_map(attn_cache, target_sbj=0, block_id=f'transformer_blocks.2', 
    ...
    attention_map = rearrange(attention_probs, 'B A (C H W) D -> B A C H W D', H=token_H, W=token_W)
    attention_map = attention_map[:,:,-1,:,:,:].sum(1).squeeze(1).permute(0,3,1,2)
